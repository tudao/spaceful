...
interface SettingsDrawerProps {
  ...
  onSwitchArchetype?: (id: string) => void;
}

const ARCHETYPES = [
  { id: 'sage',       label: 'Sage',        desc: 'Wise questions, deep reflection' },
  { id: 'coach',      label: 'Coach',       desc: 'Warm energy, action-focused' },
  { id: 'stoic',      label: 'Stoic',       desc: 'Calm clarity, practical wisdom' },
  { id: 'poet',       label: 'Poet',        desc: 'Beauty, metaphor, meaning' },
  { id: 'challenger', label: 'Challenger',  desc: 'Bold pushes, growth mindset' },
];
...
export function SettingsDrawer({
  ...
  const [activeArchetype, setActiveArchetype] = useState(currentArchetype);
  ...
  return (
    <Drawer open={open} onClose={onClose} title="Space settings">
      ...
      {/* Companion archetype */}
      <div style={{ padding: '18px 22px', borderBottom: '1px solid var(--app-border)' }}>
        <h5 style={{ fontSize: 12, fontWeight: 700, letterSpacing: '0.06em', textTransform: 'uppercase', color: 'var(--app-text-muted)', marginBottom: 12 }}>AI Companion</h5>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          {ARCHETYPES.map(a => (
            <button
              key={a.id}
              onClick={() => {
                ...
                onSwitchArchetype?.(a.id);
                toast(`Companion set to ${a.label}`, 'success');
              }}
              style={{
                display: 'flex', alignItems: 'flex-start', gap: 10,
                padding: '11px 14px', borderRadius: 12, cursor: 'pointer',
                border: `1.5px solid ${activeArchetype === a.id ? 'var(--app-accent)' : 'var(--app-border)'}`,
                background: activeArchetype === a.id ? 'var(--app-accent-soft)' : 'var(--app-surface)',
                textAlign: 'left', fontFamily: 'var(--font)', transition: 'var(--t-fast)',
              }}
            >
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: 14, fontWeight: 700, color: activeArchetype === a.id ? 'var(--app-accent-ink)' : 'var(--app-text)' }}>The {a.label}</div>
                <div style={{ fontSize: 12, color: 'var(--app-text-2)', marginTop: 2 }}>{a.desc}</div>
              </div>
              {activeArchetype === a.id && <Check size={16} style={{ color: 'var(--app-accent)', flexShrink: 0, marginTop: 2 }} />}
            </button>
          ))}
        </div>
      </div>
